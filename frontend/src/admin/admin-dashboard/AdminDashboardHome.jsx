...
const AdminDashboardHome = () => {
    const { data: stats, isLoading: isStatsLoading, refetch: refetchStats } = useGetHomeStatsQuery();
    const { data: soldPlayers, isLoading: isSoldPlayersLoading, refetch: refetchSoldPlayers } = useGetSoldPlayersQuery(undefined, { pollingInterval: 0 }); // ❌ No polling
    const { data: trendingPlayers, isLoading: isTrendingPlayersLoading, refetch: refetchTrendingPlayers } = useGetTrendingPlayersQuery(undefined, { pollingInterval: 0 }); // ❌ No polling
                        ...
                        <Card className="xl:col-span-2">
                            <CardHeader className="flex flex-row items-center">
                                <div className="grid gap-2">
                                    ...
                                    <CardDescription>Recent trending players on the website.</CardDescription>
                                </div>
                                ...
                                {soldPlayers?.data?.slice(0, 8).map((player, index) => (
                                    <div className="flex items-center gap-4" key={index}>
                                        ...
                                        <div className="grid gap-1">
                                            <p className="text-sm font-medium leading-none">{player.short_name}</p>
                                            <p className="text-sm text-muted-foreground">{player.nationality_name}</p>
                                        </div>
