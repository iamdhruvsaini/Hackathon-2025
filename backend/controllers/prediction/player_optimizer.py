...
def find_best_team(players, budget, excluded_teams=None):
    ...
    if excluded_teams:
        for team in excluded_teams:
            problem += pulp.lpSum([player_vars[i] for i in range(len(players)) if players[i]["id"] in team]) <= 10  # Ensure at least one player is different
